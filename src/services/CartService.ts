...
export const getCart = async (): Promise<Cart | null> => {
  try {
    ...
    // Chuyển đổi course_id, price và discounted_price thành số
    cartData.items = cartData.items.map((item: CartItem) => ({
      ...item,
      course_id: Number(item.course_id), // Đảm bảo course_id là số
...
export const removeFromCart = async (course_id: number): Promise<void> => {
  try {
    ...
    const response = await axiosInstance.delete(`/cart/items/${course_id}`);
    console.log('Remove from cart response:', response.data);
    if (!response.data || !response.data.success) {
      throw new Error(response.data?.message || 'Không thể xóa khóa học khỏi giỏ hàng');
    }
  } catch (error: any) {
    const errorMessage = error.response?.data?.message || error.message || 'Không thể xóa khóa học khỏi giỏ hàng';
