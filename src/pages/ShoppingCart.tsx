...
// Giả sử bạn có cách lấy token (ví dụ từ localStorage)
const getAuthToken = () => localStorage.getItem('token') || ''; // Thay bằng cách lấy token thực tế

const ShoppingCart: React.FC = () => {
  const [cart, setCart] = useState<Cart | null>(null);
  const [loading, setLoading] = useState<boolean>(true);
  const [clearing, setClearing] = useState<boolean>(false); // Thêm trạng thái clearing
  ...
  const handleClear = async () => {
    try {
      setClearing(true); // Bắt đầu trạng thái clearing
      const token = getAuthToken();
      console.log('Attempting to clear cart with token:', token);
      const response = await clearCart(); // Gọi API không cần truyền token
      ...
      setClearing(false); // Kết thúc trạng thái clearing
    }
  };
        ...
        <div className="grid lg:grid-cols-3 lg:gap-x-8 gap-x-6 gap-y-8 mt-6">
          <div className="lg:col-span-2 space-y-6">
            {cart.items.map((item: CartItem) => (
              <div key={item.course_id} className="flex items-center gap-4 bg-white p-4 rounded-lg shadow-sm border border-gray-200">
                ...
                <div className="flex items-center gap-4">
                  <div className="text-sm font-semibold text-slate-900">
                    {item.discounted_price && Number(item.discounted_price) < Number(item.price) ? (
                      <>
                        <span className="line-through text-gray-500 mr-2">${Number(item.price).toFixed(2)}</span>
          ...
          <div className="bg-white rounded-lg p-4 shadow-sm border border-gray-200">
            <ul className="text-slate-500 font-medium space-y-4 text-sm">
              ...
              <li className="flex justify-between font-semibold text-slate-900 text-base">
                Tổng cộng <span>${subtotal.toFixed(2)}</span>
              </li>
            </ul>
            ...
            <div className="mt-6 space-y-4">
              ...
              <button
                onClick={handleClear}
                type="button"
                disabled={clearing} // Vô hiệu hóa khi clearing
                className={`w-full px-4 py-2.5 text-sm font-medium ${clearing ? 'bg-gray-400' : 'bg-slate-100 hover:bg-slate-200'} text-slate-900 border border-gray-300 rounded-md transition-colors`}
              >
            ...
            <div className="mt-6 flex flex-wrap justify-center gap-4">
              ...
              <img src="https://readymadeui.com/images/american-express.webp" alt="American Express" className="w-10 object-contain" />
            </div>
          </div>
        </div>
      )}
